...
corner certifies every interior plant exactly. We widen the envelope from 1 to 4 aging/fault channels
...
Channels (each monotone in the operating region, worst-case direction in parentheses):
  R (high), cooling hA (low), thermal mass C_th (low), plating (high).

    python reproduce/dimension_independence.py
"""
import os, sys
...
print("dimension sweep (certificate is 18 iterations at every dimension):")
for d in range(1, len(CHANS)+1):
    active = CHANS[:d]; N = 150; safe = 0; worstT = 0.0
    for _ in range(N):
        theta = {k: 1.0 for k in CHANS}
        for k in active:
            lo, hi = ENV[k]; theta[k] = float(rng.uniform(min(lo, hi), max(lo, hi)))
        mT, mV, mP = drive(make_rom(theta), est); worstT = max(worstT, mT)
        safe += int(mT <= TLIM and mV <= VLIM and mP >= MARG)
    print(f"  d={d}  {str(active):40}  safe={safe}/{N}  worstT={worstT:.2f}C  cert=18 iters")

N = 500; safe = 0; worstT = worstV = 0.0
for _ in range(N):
    theta = {k: float(rng.uniform(min(ENV[k]), max(ENV[k]))) for k in CHANS}
    mT, mV, mP = drive(make_rom(theta), est)
    worstT, worstV = max(worstT, mT), max(worstV, mV)
    safe += int(mT <= TLIM and mV <= VLIM and mP >= MARG)
cornerT, _, _ = drive(make_rom(CORNER), est)
print(f"\nfull 4-channel envelope: {safe}/{N} safe, viol={100*(N-safe)/N:.2f}%, "
      f"CP95 upper={100*cp_upper(N-safe, N):.2f}%, worst {worstT:.2f}C / {worstV:.3f}V")
