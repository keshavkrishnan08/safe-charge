...
\microtypesetup{factor=1100,stretch=30,shrink=40,step=1}
\setlength{\emergencystretch}{3em}
% Typographic compression only; no content changes.
\renewcommand{\arraystretch}{0.93}      % tables only; typographic
...
ROM to a second cell chemistry (\chemTwoSafe/\chemTwoN, voltage RMSE \chemTwoV~mV).
\end{abstract}
...
\vspace{-3pt}
{\footnotesize\setlength{\tabcolsep}{0pt}\renewcommand{\arraystretch}{0.93}
\noindent\begin{tabular}{@{}p{0.78cm}@{\hspace{2pt}}p{3.24cm}@{\hspace{4pt}}p{0.78cm}@{\hspace{2pt}}p{3.24cm}@{}}
$T$, $V$ & Temperature, terminal voltage; limits \SI{45}{\degreeCelsius}, \SI{4.2}{V}.
 & $\Delta t$, $\tau_\mathrm{RC}$ & Control step, RC time constant; both \SI{30}{s}.\\
$\phi_\mathrm{an}$ & Anode plating overpotential $\phi_s{-}\phi_e$.
 & $Q_\mathrm{gen}$ & Heat generation; $Q_\mathrm{ohm}$, $Q_\mathrm{act}$, $Q_\mathrm{rev}$ its parts.\\
$I$ & Current; $I_\pi$ proposed, $I_\mathrm{cmd}$ commanded, $I^\star$ admissible.
 & $C_\mathrm{th}$, $hA$ & Thermal capacity, heat-transfer coefficient.\\
$\mathcal S$ & Certified set $\{T\le T_{\max},V\le V_{\max}\}$; $\mathcal S_{-\delta}$ tightened.
 & $R_\Omega$, $R_1$, $V_1$ & Ohmic and RC resistance; RC overpotential.\\
$g_c$ & One-step peak of constraint $c$ in the ROM.
 & $s_R$, $\gamma$ & Aging channels: resistance scale, cooling loss; $\hat s_R$ est., $s_R^\star$ rated.\\
$\delta_T$, $\delta_V$, $\delta_P$ & Control margins; $\delta_{T0}$ base thermal margin.
 & $K$, $f$ & Margin gains: resistance throttle, cooling reserve.\\
$\epsilon$, $n$ & Bisection resolution; episodes per stress test. & & \\
\end{tabular}}
\vspace{1pt}
...
$\revFrac\%$ of the irreversible heat and never overturns $\partial_I g_T\ge0$, verified numerically over $10$--$80\%$
...
gains are sized to that $20\%$-loss edge, not fitted to a failure: $T_{\max}$, $V_{\max}$, $s_R^\star$ are
manufacturer data, $f$ is derived from the fault covered, and only $K$ is set by simulation. Sweeping them over $K\in[5,25]$ and
$f\in[0.1,0.4]$ leaves zero violations in all $\gainRuns$ runs of the aged, cooling-faulted grid, a safe region, not a tuned point. The reserve also proves conservative, holding safety to a $\coolSafeLoss\%$
...
with no matrix factorization. It costs $O(\log(1/\epsilon))$ model evaluations for resolution
$\epsilon$, and the fixed iteration count meets that bound on every input: at most
$\filterEvalMax$ evaluations, \SI{\filterMemB}{B}, no dynamic allocation, and
\SI{\filterMeanUs}{$\mu$s} per projection, mean and worst case alike. A coarser tolerance keeps
$\mathrm{lo}\le I^\star$, so it only under-commands: safety is independent of $\epsilon$. Its safety logic is \filterSloc\ numpy-only source lines a
