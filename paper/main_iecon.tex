...
\SI{\dfnOursWarmT}{\degreeCelsius} against the \SI{45}{\degreeCelsius} limit; it delivers the charge of
CC--CV at 1C (SOC \oursWarmSoc) while removing that protocol's \cconeWarmPviol\% plating rate, and
holds temperature and voltage safe on \valSafe/\valN\ disjoint aged draws (Clopper--Pearson $95\%$
upper bound \valCP\%) across SOH $1.0$--$0.8$ and under a full recalibration of the ROM to a second
cell chemistry (\chemTwoSafe/\chemTwoN, voltage RMSE \chemTwoV~mV). Each projection costs at most
$\filterEvalMax$ model evaluations and \SI{\filterMemB}{B} of state, with no solver and no dynamic
allocation.
\end{abstract}
...
\vspace{-2pt}
{\footnotesize\setlength{\tabcolsep}{0pt}\renewcommand{\arraystretch}{0.98}
\noindent\begin{tabular}{@{}p{1.45cm}@{\hspace{4pt}}p{6.7cm}@{}}
...
$\delta_T$, $\delta_V$, $\delta_P$ & Control margins on temperature, voltage, plating;
$\delta_{T0}$ the base thermal margin.\\
...
$Q_\mathrm{gen}$ & Heat generation; ohmic, activation and reversible parts
$Q_\mathrm{ohm}$, $Q_\mathrm{act}$, $Q_\mathrm{rev}$.\\
...
$\epsilon$, $n$ & Bisection current resolution; number of episodes in a stress test.
\end{tabular}}
\vspace{1pt}
