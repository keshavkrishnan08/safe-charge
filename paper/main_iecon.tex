...
\input{iecon_table.tex}
\input{safety_table.tex} % consolidated (test, n, violations, CP bound) rows
\input{appendix_tables.tex} % reuse verified table-row macros (causalrows, oodsafetyrows)
...
% Caption and display spacing (typographic only; no content changed).
...
% Float packing: let floats fill more of a page top before LaTeX defers them, so
% wide figures do not leave short columns. Layout only; no content changes.
...
% Tighten inter-entry spacing in the reference list (typographic only; no entries changed).
...
fast CC--CV overshoots \SI{45}{\degreeCelsius} in every episode, peaking at
\SI{\dfnOursWarmT}{\degreeCelsius}, delivers the charge of CC--CV at 1C (SOC \oursWarmSoc)
while removing that protocol's \cconeWarmPviol\% plating, and holds temperature and voltage safe on
\valSafe/\valN\ disjoint aged draws (Clopper--Pearson \valCP\%) and under a full recalibration of the
ROM to a second cell chemistry (\chemTwoSafe/\chemTwoN, voltage RMSE \chemTwoV~mV). Each projection
costs at most $\filterEvalMax$ model evaluations and \SI{\filterMemB}{B}, with no solver.
\end{abstract}
...
\vspace{-2pt}
{\footnotesize\setlength{\tabcolsep}{0pt}\renewcommand{\arraystretch}{0.96}
\noindent\begin{tabular}{@{}p{1.4cm}@{\hspace{3pt}}p{7.35cm}@{}}
$T$, $V$ & Cell temperature and terminal voltage; limits $T_{\max}{=}\SI{45}{\degreeCelsius}$,
$V_{\max}{=}\SI{4.2}{V}$.\\
$\phi_\mathrm{an}$ & Anode plating overpotential, $\phi_s{-}\phi_e$ at the separator.\\
$I$ & Applied current; $I_\pi$ proposed, $I_\mathrm{cmd}$ commanded, $I^\star$ largest admissible.\\
$\mathcal S$ & Certified safe set $\{T\le T_{\max},V\le V_{\max}\}$; $\mathcal S_{-\delta}$ its
margin-tightened inner approximation.\\
$g_c$ & One-step peak of constraint $c$ predicted by the ROM.\\
$\delta_T$, $\delta_V$, $\delta_P$ & Control margins on temperature, voltage, plating; $\delta_{T0}$
the base thermal margin.\\
$s_R$, $\gamma$ & Aging channels: resistance scale and cooling-loss factor; $\hat s_R$ estimated,
$s_R^\star$ the rated end-of-life bound.\\
$K$, $f$ & Margin gains: resistance throttle and cooling reserve.\\
$\Delta t$, $\tau_\mathrm{RC}$ & Control step and RC time constant, both \SI{30}{s}.\\
$Q_\mathrm{gen}$ & Heat generation; ohmic, activation and reversible parts $Q_\mathrm{ohm}$,
$Q_\mathrm{act}$, $Q_\mathrm{rev}$.\\
$C_\mathrm{th}$, $hA$ & Lumped thermal capacity and heat-transfer coefficient.\\
$R_\Omega$, $R_1$, $V_1$ & Ohmic resistance, RC-branch resistance and its overpotential.\\
$\epsilon$, $n$ & Bisection current resolution; episodes in a stress test.
\end{tabular}}
\vspace{1pt}
...
$\sim0.37$ of the RC overpotential energy after the cut in continuous time, which the margin $\delta_T$
(base \SI{0.5}{\degreeCelsius} plus a $\sim$\SI{4.25}{\degreeCelsius} cooling reserve) absorbs; the
discrete update leaves less still, so the bound is conservative. Two opposing requirements pin the step
rather than tuning: the residual argument wants $\Delta t\gtrsim\tau_\mathrm{RC}$, while the explicit
discretisation of the RC branch is stable only for $\Delta t\le\tau_\mathrm{RC}$. Safety is not fragile
in the choice: sweeping $\Delta t$ from \SI{\stepLo}{s} to \SI{\stepHi}{s} yields no violation, moves
delivered SOC by under $\stepSocSpread$ points, and never lets the residual exceed $\rcResidPct\%$ of
the base margin. The invariance is exact in the
...
(\SI{2}{C} at $\SI{25}{\degreeCelsius}$ tapering to \SI{1.0}{C} at $\SI{10}{\degreeCelsius}$ with a
\SI{\platCapFloor}{C} floor, fit from
...
$\revFrac\%$ of the irreversible heat over the admissible band and never overturns $\partial_I g_T\ge0$,
which holds for every $I$ above \SI{\monoImin}{C}, an order of magnitude below the cap's
\SI{\platCapFloor}{C} floor, verified numerically over $10$--$80\%$ SOC. Each $g_c$ is thus non-decreasing in $I$ and in each channel, so $I^\star$ is non-increasing in each,
...
gains are sized to that $20\%$-loss edge, not fitted to a failure. Of the design constants,
$T_{\max}$, $V_{\max}$ and $s_R^\star$ are manufacturer limits and the rated end-of-life envelope,
$f$ is derived from the cooling fault to be covered, and only $K$ is chosen by simulation. Sweeping
$K\in[5,25]$ and $f\in[0.1,0.4]$ leaves zero violations in all $\gainRuns$ runs of the aged,
cooling-faulted grid, a safe region, not a tuned point; $K$ is then set by delivered charge, which it
costs monotonically, rather than by safety. The reserve also proves conservative, holding safety to a $\coolSafeLoss\%$
...
\begin{figure*}[!t]\centering
\includegraphics[width=\textwidth]{fig3_aging}
\caption{Deployment across the SOH range. \textbf{(a)}~among the filtered variants, only ``no
adaptation'' violates, and early
($55\%$ of episodes at SOH~0.9, $100\%$ at 0.8). Oracle, worst-case-$R$ prior, and our estimator
...
\begin{figure*}[!t]\centering
\includegraphics[width=\textwidth]{fig10_monotone}
...
with no matrix factorization. It costs $O(\log(1/\epsilon))$ model evaluations for current resolution
$\epsilon$, and because the iteration count is fixed rather than convergence-tested, that bound is met
on every input: at most $\filterEvalMax$ closed-form evaluations, ${\sim}\filterFlops$ flops, and
\SI{\filterMemB}{B} of state and constants with no dynamic allocation, and
\SI{\filterMeanUs}{$\mu$s} per projection in the reference implementation, where mean and worst case
coincide to within host scheduling noise because the work per call is fixed, orders of magnitude
inside the \SI{30}{s} step. The
tolerance after $k$ halvings is $I_{\max}/2^{k}$, \SI{\tolEpsUA}{$\mu$A} at $k{=}\tolIters$; since
the bisection maintains $\mathrm{lo}\le I^\star$, a coarser tolerance only under-commands, so safety is
independent of $k$ and only delivered charge varies, saturating by $k{\approx}\tolKnee$. Its safety
logic is \filterSloc\ numpy-only source lines a functional-safety audit can read in full, where an embedded QP instead places a compiled ADMM solver
(\qpTcbFiles\ bundled C source and header files) in the trusted computing base. Against a receding-horizon MPC ladder,
...
filter's. Because it is a bound and not an estimate, it covers what it bounds and degrades gracefully
past it: raising the true resistance scale beyond the rated $s_R^\star$ leaves every in-envelope cell
safe with margin in hand, and the first violation appears only at $s_R{=}\eolBreakSR$,
$\eolBreakX\times$ the rated bound and far beyond the point a BMS retires the cell. Unique to the filter is Prop.~\ref{prop:inv}, replacing the solver with a fixed
...
and a pack-level extension are the natural next steps, in that order: hardware validation is what
converts a ROM-relative certificate into a rated one, and a predictive plating constraint is what would
move plating from enforced to certified. We release the filter, ROM, executable
